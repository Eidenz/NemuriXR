<script lang="ts">
  ...
  import InviteMessageEditor from "$lib/components/InviteMessageEditor.svelte";
</script>
...
>
  ...
  {:else if app.config}
    ...
    <div class="rows">
      ...
      <InviteMessageEditor
        kind="message"
        title="Send an invite message"
        desc="Attach one of your VRChat invite messages when accepting"
        bind:enabled={aa.invite_message_enabled}
        bind:slot={aa.invite_message_slot}
        onchange={save}
        placeholder="e.g. Sleeping — come on in! zzz"
      />

      <InviteMessageEditor
        kind="requestResponse"
        title="Send a decline message"
        desc="Reply to requests that aren't accepted with a message"
        bind:enabled={aa.decline_message_enabled}
        bind:slot={aa.decline_message_slot}
        onchange={save}
        placeholder="e.g. Sleeping right now — catch you later!"
      />
    </div>
